...
describe('action', () => {
  ...
  const program = anchor.workspace.vault_dapp
  ...
  it('should deposit tokens to vault', async () => {
    ...
    expect(vaultTokenAccountBalance.value.uiAmount).equal(1)
  })
  ...
  it('should withdraw tokens from vault', async () => {
    ...
    expect(vaultTokenAccountBalance.value.uiAmount).equal(0.5)
  })
  ...
  it('lock the vault', async () => {
    ...
    expect(vaultState.isLocked).equal(true)
  })
  ...
  it('unlock the vault', async () => {
    ...
    const vaultState = await program.account.vaultState.fetch(vaultPda)
    expect(vaultState.isLocked).equal(false)
  })
  ...
  it('transfer ownership', async () => {
    ...
    expect(vaultState.owner.toBase58()).equal(newOwner.publicKey.toBase58())
  })
})
